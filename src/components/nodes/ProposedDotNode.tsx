...
export type ProposedDotType = Node<BitcoinNodeData, "proposedDot">;

export function ProposedDotNode({ data, selected }: NodeProps<ProposedDotType>) {
  const dotColor = (data.color as string) || "#6B7280";

  return (
    <div className="flex flex-col items-center cursor-pointer group">
      <div className="flex items-center gap-1 mb-1">
        <span className="w-1.5 h-1.5 rounded-full bg-yellow-500" />
        <span className="text-[9px] font-medium text-yellow-600 dark:text-yellow-500/70">proposed</span>
        ...
        {data.bips && (data.bips as number[]).length > 0 && (
          <div className="flex gap-0.5 mt-1">
            {(data.bips as number[]).map((bip) => (
              <span key={bip} className="text-[8px] font-mono px-1 py-px rounded bg-gray-500/10 text-gray-500 dark:text-gray-400">
                BIP{bip}
              </span>
            ))}
          </div>
